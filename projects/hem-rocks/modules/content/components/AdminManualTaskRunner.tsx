...
function migrate(allContentItems: IContentItem[]) {
  ...
  const albums = allContentItems.filter(item => hasTag(item, 'albums'))

  for (const oldItem of allContentItems) {
    const newItem = Object.assign({}, oldItem)

    if (newItem.secondaryAttribution === 'Katathymie') {
      newItem.slug = slugify(newItem.title + '-' + newItem.attribution)
    }
